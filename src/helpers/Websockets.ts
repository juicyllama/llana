...
import { CACHE_DEFAULT_WS_IDENTITY_DATA_TTL } from 'src/app.constants'
...
export class Websockets {
	...
	async afterInit(client: Socket): Promise<void> {
		client.use(async (socket: any, next) =>{
			...
			try {
				...
				this.logger.error('Socket Auth Failed - Unauthorized')
				return next(new Error('Forbidden'))	
			}
		})
	}
	...
	async publish(schema: DatabaseSchema, type: SocketType, id: number | string): Promise<void> {
		...
		const clients = Array.from(this.server.sockets.sockets.values()).map((socket) => { return socket.id })
		...
		for (const client of clients) {
			...
			if(auth.valid){
				this.logger.debug(`[Websockets] Skipping ${schema.table} ${type} for #${id} to ${client} due to lack of permission`)
				continue
			}
